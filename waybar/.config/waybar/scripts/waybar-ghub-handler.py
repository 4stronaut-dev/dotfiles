...
try:
    ...
    devices = [] # initialize device list
    ...
    tooltip += '\nList of connected devices and their features:\n'
    for device in devices:
        tooltip += f'{type_icon_map[device['Kind']]}: {device['Name']}\n'
        tooltip += 'Features:\n'
        for feature in device['Features']:
            tooltip += f'\t{feature['FID']}:\n'
    ...
    waybar_input['percentage'] = int(devices[device_index]['Features'][0]['Battery'].split(',')[0].rstrip('%'))
    waybar_input['class'] = devices[device_index]['Features'][0]['Battery'].split(',')[1].split('.')[1].lower()
